...
- `cargo run --release --bin arena -- [対局数] [戦略A] [戦略B]` — 戦略同士のローカル対戦。
  戦略の変更は必ずこれで旧戦略に有意に勝ち越すことを確認する
  （例: `cargo run --release --bin arena -- 200 estimator heuristic`）
...
- 思考時間の壁時計は測らないので、戦略側は粒子数・リプレイ回数・時間予算で
  自ら打ち切る構造を保つこと（フィッシャー300秒+3秒 → 1手1〜2秒が目安）
