...
- `cargo build` / `cargo run --release` — 実行には環境変数が必要:
  ...
  - `TSUITATE_GEN_NONPROMOTE`（**既定 on**、0 で従来の「成れるなら成る」）/
    `TSUITATE_PROMO_RISK_PREROLE`（**既定 on**、0 で成った後の駒種へ切り戻し）/
    ...
    **不成の候補生成と、成る手の取られリスクの価格**（2026-08-08、
    ...
    - PREROLE = リスクは持ち込んだ駒の価値で数える（1手読み own_risk と
      ...
      20/20**
    - **不成候補の value NN は成り側の双子で評価する**（ノブなし = GEN 有効時
      のみ発火）: NN の学習データは自動成りのみで成/不成の差分は分布外ノイズ
    - **CAPTURE_RETREAT**: 捕獲直後の厳密手戻り（不成のみ）を backtrack 免除＋
      `w×交換価値` 加点。GEN+PREROLE で沈んでいた m024 の 3b4b（飛車退避）を
      回復（実測 2.0→8.0）。成り手戻りは除外（m087/m089 の 4a3b+ を押し上げる）
    - **2026-08-11 採用**（quest31 得点 5.5 超え作業）: combo
      （LINK_ENDGAME_DAMPEN=40 / HAND_ASSET_W=0.5 / MATERIAL_DEGEN_Q0=0.3 /
      KING_KNOWN_APPROACH_W=2 / PROMOTE_FAR_W=1.0）+ GEN + PREROLE +
      CAPTURE_RETREAT=0.08 でクリーン suite 145件×5seed **avg 5.530**
      （対照 4.717、+0.81）。凍結版はこの名前を知らない
  - `TSUITATE_LINK_ENDGAME_DAMPEN`（**既定 40**、0 で無効）— ブラインド終盤の
    紐減衰。`link_w /= 1 + w × endgame_push`
  - `TSUITATE_HAND_ASSET_W`（**既定 0.5**、0 で無効）— 持ち駒の資産損
    （仕事のない打ちへ `w×交換価値`）
  - `TSUITATE_MATERIAL_DEGEN_Q0`（**既定 0.3**、0 で無効）— 観測裏付けの無い
    捕獲の駒得を粒子質量で縮める退化ゲート
  - `TSUITATE_KING_KNOWN_APPROACH_W`（**既定 2**、0 で無効）— 既知脅威マスへ
    近づく玉の手への減点
  - `TSUITATE_PROMOTE_FAR_W`（**既定 1.0**、0 で無効）— 玉候補から遠い大駒成り
    への減点
