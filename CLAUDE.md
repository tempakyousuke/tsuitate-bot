...
- `check.rs` — 王手中の回避手選択のための制約推論（`CheckSolver`）。
  ...
  角の王手を無条件で捕獲できるのに0/20回しか選ばなかった）。対策として
  `captures_checker`（王手駒仮説マスへの捕獲か判定）を追加し、該当候補には
  `strategy.rs::CHECK_CAPTURE_P_LEGAL_FLOOR`（0.35）の下限を敷く
  （2026-07-21、estimator_v8で凍結。捕獲選択率0/20→10-11/20に改善。
  根本原因はもう一つあり、王手駒についての粒子ビリーフ自体の較正不良
  （真の王手駒への信念1.7%等）は未着手。NN化ロードマップの段階①②
  （likelihood.rs/opp_move_weightのNN置換・信念ネット）が対象領域）
