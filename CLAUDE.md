...
- `cargo build` / `cargo run --release` — 実行には環境変数が必要:
  ...
  - **quest31 得点5.5超えコンボ（PR#1、2026-08-12 計測で既定0のまま導入）**:
    ...
    - **シナリオは大幅改善で本物**: suite 不合格計 1258→912・平均得点
      4.528→**5.274**（+0.75。ただし目標 5.5 は20試行では未達 = PR 主張の
      5.669 は5シードの楽観値）。長年の「3三角成固執」が初めて大きく動いた
      （m111/m121/m123/m127 が +6.3〜+8.4）。未採点85件（≥2回の12件は
      eval に ? 追記済み、採点待ち）
