...
- `cargo build` / `cargo run --release` — 実行には環境変数が必要:
  ...
  - `TSUITATE_CHECK_KING_GAIN_MEAN`（既定 on、0 で従来挙動）— **王手中の玉の手の
    ...
    揃える。実測（20試行）: king-evade 反則 57→40・dragon-check-drop 19→14・
    kakutori 16→37（捕獲2/20→11/20 のプローブ回帰込み）。アリーナ判定待ち
