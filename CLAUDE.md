...
- `cargo run --release --bin arena -- [対局数] [候補] [基準1] [基準2] ...` — 戦略同士のローカル対戦。
  ...
  勝ち越すことを確認する（例: `cargo run --release --bin arena -- 1000 estimator estimator_v1`）。
  50%付近の信頼区間は 200局で±7pt / 1000局で±3.1pt — 数ptの改善検証には1000局以上必要
...
- `frozen/` — アリーナ比較の基準となる凍結版戦略（`estimator_v1` = 2026-07-06 時点のコピー）。
...
- **比較の基準は heuristic ではなく凍結版**（`estimator_v1` 等）。heuristic への勝率は
