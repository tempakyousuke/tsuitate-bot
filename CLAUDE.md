...
- `cargo run --release --bin rank_probe -- <kif|名前> <ply> [シード数=5] [focus手,...]` —
  その局面の候補手ランキング内訳（score/gain/p_legal/foul_cost/adjust と
  除去EV・賭けpen・詰めろ2項）を CLI へ。GUI を立てずに評価項の効きを見る
- `cargo run --release --bin belief_probe -- <kif|名前> <ply> <マス,...> [シード数=3] [マス:駒種]` —
  手番側の粒子が**そのマスに何がいると思っているか**（玉位置ビリーフの任意マス版）。
  「この手を選んだのは相手駒の位置を誤解しているからでは？」を潰すための道具で、
  全粒子と厳密粒子の両方の割合を出す。第5引数（例 `8i:Knight`）でその仮説を持つ
  粒子の盤面・持ち駒を実際に印字できる。**信念の漏れか合法な仮説かはここまで
  見ないと判別できない**（2026-07-25 の実例: 8九に先手桂を置く0.2%の粒子は、
  後手が8九で取った桂を先手が4七で取り返して8九へ打ち直した合法な筋だった）
...
kakudo 正解シード1/4→3/4）。次の候補: 段階②（信念ネット）。
