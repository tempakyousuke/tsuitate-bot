...
- `cargo build` / `cargo run --release` — 実行には環境変数が必要:
  ...
  - `TSUITATE_LAST_FOUL_GUARD_2`（既定 **0**、作業点 36）/
    ...
    **単独では一度もアリーナ計測されていなかった**ので 2026-08-19 に測った。
    ...
    - vs v14 のペア: seed 20260815（104局）**57.7% vs 52.9%（+4.8）**、
      20260816（200局）**53.8% vs 45.7%（+8.1）**、
      20260817（200局）47.6% vs 50.0%（−2.4）。
      合計 504局で **53.8%（271勝）vs 50.4%（254勝）= +3.4pt**
      （差の 95% CI ≒ ±6.2pt なので有意ではない）。
      反則/局は 5.88〜6.30 vs 6.09〜6.23
    **既定0のまま保留**（2026-08-19）。2勝1敗・合計 +3.4pt は
    quest31 ロールバックでユーザーが定めた手順の ③（ガントレット v9〜v14 で
    全基準に有意勝ち越し）を通していない。有効化は
