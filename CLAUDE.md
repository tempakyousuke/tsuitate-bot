...
- `TSUITATE_THINK_BUDGET_MS=2000 cargo run --release --bin check_belief_probe -- [--seeds 3]
  ...
  **王手駒仮説の希釈 P0-1: runtime の伝達の分解**（issue #36、2026-08-29）。
  ...
  - **P0-2 の初回実測（run 33298525370）は撤回**（2026-08-30、PR #37 レビュー5巡目 [P1]）。
    実再決定 arm を**タグ順の固定順**で回していたので `current@real` が常に先・
    `oracle@kinf@real` が複数 arm を挟んだ後になり、**主差に実行順効果が残りうる**
    （実再決定だけが壁時計デッドラインまで粒子を回す。shadow は決定論的なので
    無関係）。`arm_order` が無いので事後監査もできず、記録は schema 4 で
    新しい入力契約も通っていない。**AB/BA を閉じた schema 5 で取り直すまで
    採否に使わない**（効果量は大きいが「大きいから大丈夫」は #31 P0-7 で
    否定された読み方 = 並走交絡を外したら層別の形が全部消えた）。
    以下は撤回した参考値:
    - 主 estimand（**全王手手番の自然頻度**・単王手のみ・元対局 cluster CI）
      `R_foul = current@real − oracle@kinf@real` が
      **v13 +0.3289 [+0.2604, +0.4013]（94局）/ v14 +0.3299 [+0.2770, +0.3902]（99局）**、
      **合算 +0.3294 [+0.2859, +0.3744]**、veto（各相手で正）OK。
      安全性3つは margin の内側どころか**改善側**（破滅率 +0.0000・受理率 +1.85pt・
      即時反則負け −1.85pt）
    - **反則した手番では 1.38 → 0.36 反則/手番**（v14、Δ −1.02 [−1.18, −0.87]、
      受理率 91.6 → 97.6%）。**k に単調**（k2 −0.14 / k4 −0.32 / k8 −0.51 / kinf −1.02）
    - **実再決定は p-only より強い**（0.36 vs 0.54）＝ gain と `removal_term` の
      作り直しぶんが効くので、**P1 を p だけに入れる設計は上限を取り逃がす**
    - **#31 の α が動かせなかった型2（kakutori）が信念側からは 1.32 → 0.17**
      （v13 も 1.36 → 0.11）。型1 は 1.61 → 0.36。「一律に掛けると型2 の捕獲プローブまで
      沈む」という #31 の懸念は、**沈めるのでなく当てにいく信念側では起きない**
    - **誤誘導 arm は壊す**（+1.46 [+1.05, +1.84]・破滅 8.6%）＝効いているのは
      「真の仮説を強めたから」であって倍率のせいではない、の対照
    - 反則0の手番は **0.45 → 0.43（Δ −0.03 [−0.07, +0.01]）**で非劣性〜弱く改善。
      新しい介入を足す害は無い
    - 判定以前の関門は全通過: 恒等対照 `oracle@k1@shadow` が **bit-exact 0.0000**、
      **再決定のノイズ床（`oracle@k1@real − current@real`）が +0.00 [−0.00, +0.01]**
      （レビュー2巡目で `current@real` を同じ再ランキング経路へ寄せた効果。
      寄せる前は p の最大差 0.0071・4回で 0.006〜0.085 と揺れていた）＝
      **主 arm の差はほぼ全部が介入によるもの**、`deduce_last_move` の真仮説落としは 0
    - **これはオラクル = 上限**であって、学習可能な事前がここへ届くとは言っていない。
      P0-1 の coverage failure 54.0% / 43.6% を分母に含めたままこの効果量である点が
      むしろ所見（救えない半分を含めて +0.33）
    - 副産物: **β の近似通過条件は今回も不通過**（|shadow − real| 0.212 / 0.098 に対し
      β の改善量 +0.066 / +0.007 で比 3.23 / 13.67、門は 0.25 以下。#31 の 1.88 / 2.14 と
      同じ結論）。**候補分布上の較正も #31 P0-5 と一致**（捕獲試み +0.035 / +0.029、
      最も過信なのは玉の手 +0.090 / +0.074）で **γ は引き続き指示されない**
