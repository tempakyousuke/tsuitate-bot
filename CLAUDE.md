...
- `cargo build` / `cargo run --release` — 実行には環境変数が必要:
  ...
  - `TSUITATE_CHECK_BELIEF_OCC_W`（既定 **1.0**、0 で従来挙動 = 切り戻しノブ）—
    **王手駒仮説の占有事前**（v15 候補、2026-08-19）。CheckSolver の仮説は
    ...
    捕獲の `p_legal` が薄まる（kakutori 残ギャップ 8/20 の構造）。pointwise
    信念ネットは「今どの駒が王手しているか」は表現できないが、**空きマスに
    王手駒は居ない**という必要条件は占有較正（対数損失 0.36 / AUC 0.89）で
    粒子より当たる。仮説重みへ `(1−w)+w·p_occ` を掛け、床 0.05 で仮説を殺さない
    （健全性）。捕獲マスブーストの後・粒子投票の**前**に掛ける。直前の捕獲マスは
    観測で占有確定なので乗らず、粒子投票は上書きできる。
    p_legal への直接供給（反則マス記憶系が全滅したチャネル）
...
- `check.rs` — 王手中の回避手選択のための制約推論（`CheckSolver`）。
  ...
  **v15 候補（2026-08-19）: 信念ネット占有を仮説の事前にする**
  ...
  仮説重み × 占有（床 0.05）で幻仮説を薄め、粒子投票より前に掛ける。
  直前の捕獲マスは乗らない。アリーナ未計測・凍結版は知らない
