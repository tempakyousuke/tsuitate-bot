...
- `TSUITATE_THINK_BUDGET_MS=2000 cargo run --release --bin check_belief_probe -- [--seeds 3]
  ...
  **王手駒仮説の希釈 P0-1: runtime の伝達の分解**（issue #36、2026-08-29）。
  ...
  - **P0-2 の実測（run 33306228500、AB/BA を閉じた schema 5・4 seed・2000ms・
    ...
    - **P0-3（学習）へ進まない**: P0-4 の門は `R_learned / R_oracle ≥ 0.5` なので、
      **上限で +0.034 しか出ない以上、学習可能な事前がその門を通る余地が無い**
    - **#28 / #31 / #34 と同じ署名が今度は上限で出た**（#28 `Δpolicy` 最大 +0.0385 /
      #31 `alpha@k2` +0.0060 / #36 +0.0343、いずれも門 0.04）。
      「王手中の反則を減らす」方向は価格でも信念でも**勝率への変換率が低い**、
      という読みが2経路から支持された。**H1 は棄却も支持もされていない**
      （仮説重みを直せば反則は減る = P0-2 の +0.334。減った反則が勝率にならない）
