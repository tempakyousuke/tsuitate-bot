...
- `cargo run --release --bin mine_check -- [--min-fouls N] [--emit <dir> <接頭辞>] <records/*.jsonl...>`
  ...
  **仮説分布の診断**（2026-08-19 追加）: `TSUITATE_DEBUG_CHECK=1` は候補ごとの
  `prior/gain/p_legal/foul_cost/score` に加えて、決定点の頭で
  `DEBUG hypotheses(N): <マス><駒種>=<正規化重み> ...`（重みの降順、上位14件）を
  出す（`CheckSolver::debug_hypotheses`）。「正しい解消手の p_legal が薄い」局面で
  どの幻仮説が質量を持っているかを見るための一次情報
...
- `cargo build` / `cargo run --release` — 実行には環境変数が必要:
  ...
  - `TSUITATE_CHECK_HYP_PRIOR`（既定 **0** = 従来の一様、作業点 1.0）/
    `TSUITATE_CHECK_FOUL_EVIDENCE`（既定 **0**、作業点 1.0）/
    `TSUITATE_CHECK_ESCAPE_DECAY`（既定 **1.0** = 従来、作業点 0.6）/
    `TSUITATE_TURN_FOUL_ESCALATION`（既定 **0**、作業点 0.5）—
    **v15 の被王手まわり4件。すべて実測して既定オフのまま**（2026-08-19。
    全計測は docs/improvement-plan-2026-08-19-v15-foul-economy.md）。
    発端は `bin/mine_check` が採掘した被王手の決定点12件
    （`scenarios/arena-check-*.kif`。1手番で4〜7回の反則を焼く実例）と、
    アリーナ104局のシャード実測の反則内訳
    **打ちマス 77 / 王手解消失敗 72 / 経路 18 / 飛び込み 1**（26局）。
    - `CHECK_HYP_PRIOR` = 王手駒仮説の**マス単位正規化**。仮説は（マス,駒種）の
      直積で重み一様なので **P(マス) ∝ そのマスから王手になりうる駒種の個数**に
      なり、自玉の隣接（8〜10通り）が離れた飛び駒（2通り）より構造的に 4〜5 倍重い。
      arena-check-foul01 の実測（`TSUITATE_DEBUG_CHECK=1` に追加した仮説ダンプ）:
      2一の8仮説が質量 0.33、真の 1一（打たれた飛車）は 0.048、正解の合駒
      `N*2a` の p_legal は 0.041 で7連続反則。**指摘は正しいが均すのは過剰補正**で
      suite 959→1033・アリーナ 52.9→48.1%・kakutori 10→7/20 と全面的に悪化
    - `CHECK_FOUL_EVIDENCE` = 反則の証拠の強さを手の種類ごとの実測合法率へ
      （玉の逃げ 0.22 / 玉での仮説マス捕獲 0.27 / 非玉の捕獲 0.03 / 打ち 0.05。
      一律 0.15 だった）。`resolve_probability` は触らない（2026-07-29 に悪化した経路）。
      被王手シナリオの追加反則は 1890→1594（−16%）だが suite 959→1022・
      アリーナ 51.0%
    - 上記2つの併用は被王手の追加反則 **1890→1304（−31%）**・アリーナ 55.8%
      （+2.9pt）だが suite 959→1005。**王手中のバーストは反則総量の一部**で、
      アリーナの反則/局は 6.22→6.14 しか動かない
    - `CHECK_ESCAPE_DECAY` = この手番で玉の手が反則になった回数だけ、残りの
      玉の手の解消確率を `decay^n` する（見えない第2の駒が逃げ先を覆う場合の
      相関を後付けで入れる）。foul01 では仮説事前と組んでも反則数は変わらず、
      アリーナ未計測のまま既定オフ
    - `TURN_FOUL_ESCALATION` = `foul_cost *= 1 + w×この手番の反則回数`
      （同一手番の反則はモデルが外れている直接の証拠）。反則0回の決定は不変。
      アリーナ 55.8%・**反則/局 6.22→5.74（全構成で最小）**・平均手数 96.8→92.6
      だが suite 959→1030
    **読み**: 反則/局を下げた構成（5.74 / 5.88 / 6.14）が 55.8〜57.7%、
    上げた構成（6.37）が 48.1% と、5点の (反則/局, 勝率) はほぼ単調。
    **効くのは仮説の較正ではなく「反則の値段」側**だった。
    104局は ±9.6pt なので、単発の104局で採否を決めないこと
  ...
  - `TSUITATE_LAST_FOUL_GUARD`（**既定 60**、0 で従来挙動 = 切り戻しノブ）—
    ...
    **以後の対照は v14 凍結時（2026-08-19、run 32155893612、既定 env・20試行）
    の 1004 / 5.197**（未採点39）
  - `TSUITATE_LAST_FOUL_GUARD_2`（既定 **0**、作業点 36）/
    `TSUITATE_LAST_FOUL_GUARD_3`（既定 **0**、作業点 16）— 残り反則2回/3回の床。
    上の残り1回の床（60）の続きだが、quest31 6.0 の束に混ざったまま既定0へ戻され
    **単独では一度もアリーナ計測されていなかった**ので 2026-08-19 に測った。
    凍結版も同じ env 名を読むので `-f env=` では両側が動く。**候補は既定値を
    作業点へ動かした commit、対照は同じ commit に `..._2=0 ..._3=0` を渡す**
    （v14 の既定も0なので基準側は不変）というペアで計測する。
    - suite（全件20試行、同一コミット）: **956 / 5.231** vs 対照 959 / 5.305 =
      中立。機構指標は明快で **foul_limit が出るシナリオが 11 → 3**、
      採掘した被王手シナリオの追加反則も 1890 → 1763
    - vs v14 のペア5シード: 20260815（104局）57.7 vs 52.9（**+4.8**）/
      20260816（200局）53.8 vs 45.7（**+8.1**）/ 20260817（200局）47.6 vs 50.0（−2.4）/
      20260818（200局）54.3 vs 52.9（+1.4）/ 20260819（200局）52.4 vs 52.9（−0.5）。
      **合計 904局で 54.5%（493勝）vs 52.4%（474勝）= +2.1pt**
      （差の 95% CI ≒ ±4.6pt）。反則/局は 5.62〜6.48 vs 5.92〜6.35
    **不採用・既定0のまま**（2026-08-19）。3シード目までは +3.4pt と有望に見えたが、
    5シードまで伸ばすと +2.1pt（3勝2敗）で CI 内。残り1回の床（60、3シード全勝
    +5.3pt）とは別物。有効化は
    `TSUITATE_LAST_FOUL_GUARD_2=36 TSUITATE_LAST_FOUL_GUARD_3=16`。
    **副産物（重要）**: 同一構成・同一基準の 200局ペアが seed によって
    −2.4 〜 +8.1pt に散った。**200局でも ±7pt は素で動く**ので、
    +3pt 級の効果を 1〜3 シードで判定してはいけない
