  ...
  **反則するたび更新しながら真実で裁定**する。**runtime には何も入らない**。
  ...
  - **実測（run 33171943105、発見セット・4 seed・2000ms。v14 163決定点＋対照163 /
    ...
    - **β は中止**: 近似通過条件が**両相手で不通過**（近似誤差/改善量 =
      **0.58（v14）/ 0.33（v13）**、門は 0.25 以下）。正確版（候補ごとに推定器を
...
- `TSUITATE_THINK_BUDGET_MS=700 cargo run --release --bin check_price -- [--seeds 2]
  ...
  **王手中の反則経済 P0-7: 反則トークンの影の価格**（issue #31、H1 の主測定）。
  ...
  - 層別は 残り反則 × 手数帯 × **相手の残り反則**（効果修飾）。CI は元対局単位の
    cluster bootstrap
  - **実測（run 33171943105、発見セット・2 seed・700ms・1局2点。v14 372ペア/98局・
    v13 328ペア/88局）**: 全体は **−0.0161 [−0.0829,+0.0528]（v14）/
    −0.0610 [−0.1327,+0.0092]（v13）** でどちらも負だが CI は 0 を跨ぐ。
    **形が出るのは残り反則ではなく手数帯と相手の残り反則**:
    - **中盤(50-89) は両相手とも CI が 0 を跨がず負**（v14 −0.0933
      [−0.1753,−0.0068] / v13 −0.1308 [−0.2279,−0.0364]）。序中盤・終盤は ~0
    - **相手が上限間際（残り0-3）だと価格は正**（v14 +0.1364 / v13 +0.2059
      [+0.0333,+0.4000]）= 自分の1反則がタダになる。既存の `foul_diff_pow`
      （相手の残数で割り引く項）は方向として裏づけられた
    - **残り反則別はどのバケットも CI が 0 を跨ぎ単調でもない**（n=6〜96 と小さい）。
      「残数依存が無い」ではなく「**この標本では検出できない**」と読む
    - 現行 `foul_cost` は残数の単調増加関数で手数帯の項が無いので、**形が違う**
