...
- `cargo build` / `cargo run --release` — 実行には環境変数が必要:
  ...
  - `TSUITATE_GEN_NONPROMOTE`（既定 0 = 従来の「成れるなら成る」）/
    ...
    **不成の候補生成と、成る手の取られリスクの価格**（2026-08-08、
    ...
    - **未解決2件**: ①m024 が 2→20/20 の実回帰（24手目は「飛車を逃す」
      ...
      suite は 933/4.889 の微負。**採否は新出手（銀不成 3h4i×2〜5 等）の
      採点後に判断**（見かけの suite 改善は C1 と同じ指標穴を含む）
  - `TSUITATE_BLIND_HOME_RISK_W`（既定 0 = 無効）— **ブラインド進入リスク**
    ...
    - **採否は保留**（2026-08-09 時点）: w=0.5 の改善には未採点の新出手
      （m121/m123 の S*2b×13 等、仮4点）が含まれ、採点次第で反転しうる
      （C1型の指標穴）。ユーザーの追記分採点後に確定値で判定する。
      過課金側の改良案 = 「相手が動かした駒数（観測から既知）で home 残存
      確率を枚数ベースで割り引く」
