...
| `mate-economy-p0.md` | 計画/記録 | 2026-08-27。issue #28 の P0。計測基盤・P0-3・P0-6 が入り CI で実測まで取ったが、**初回計測（run 33055734354）は PR #30 のレビューで撤回**（q を別に作り直した推定器で測っていた・継続の乱数が arm ごとに違った）。修正して再計測中。暫定の傾向は「Δoracle は上限として実在するが Δpolicy が必要条件に届かない」 |
