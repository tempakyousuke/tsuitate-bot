...
| `mate-economy-p0.md` | 計画/記録 | 2026-08-27。issue #28 の P0。計測基盤・P0-3・P0-6 が入り、CI で実測（初回 run 33055734354 は PR #30 のレビューで撤回、再計測 run 33073983617 が正）。**判定は「P1 へ進まない」**: `Δoracle` +0.111（正直版 +0.067・実質 +0.087）で上限は実在し検出も適合率 0.99 だが、`Δpolicy` +0.024 が必要条件 0.04 に届かず、taint で argmax を 51% まで受けへ寄せても w=0 対照から +0.010。律速は受けの検出でなく**受けた後の反則経済**。P0-1 の時系列・P0-4・P0-5 は残り |
