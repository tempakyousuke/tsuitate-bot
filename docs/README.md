...
| `check-economy-p0.md` | 計画/記録 | 2026-08-28。issue #31 の P0。**P0-1〜P0-4 が実装済み**（P0-1 型の分類 / P0-2 手種別・順番別の較正 / P0-3 ソルバー方策の破滅の原因は粒子不要、P0-4 の w\* 監査は `bin/check_probe` で粒子を回す）。P0-5〜P0-7 は未実装。判定の門（方向つき較正誤差の cluster CI 下限 > 0.1・両相手で同方向）と、手数の対応・分母の規約はこの文書 |
