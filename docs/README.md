...
| `check-prep-p0.md` | 計画/記録 | 2026-08-29。issue #34 の P0。**P0-1（連続王手の起点）と P0-2（較正プローブ）を実装・計測**（Check prep run 33247289035 / 33247660080、発見セット = Arena run 32697854659）。段1（構成概念検証・適合率 0.95）と段2（発火母数 95〜99%）は両相手で通過するが、**段3の主 estimand が両相手とも 0 を跨ぐ**ので「この形のままでは検証セットへ送らない」。**実王手起点の 83% は隣接（距離1）**で、F1（距離2以上）の被覆は負・隣接の被覆は正と**符号が逆**＝「王手起点の被覆」は単一の構成概念として成立しない。P0-3 / P0-4 は未実装 |
