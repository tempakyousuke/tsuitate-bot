...
P1 へ進まずに閉じ（**#28 / #31 は「機構は動く・反則は減るが勝率の門に届かない」、
#34 は「そもそも分離しない」で反則・勝率とも未測定**）、3件の docs の「次にやるなら」が
...
### 所見: #28 / #31 と同じ形が、今度は**オラクル上限で**出た

| issue | 機構 | 反則 | 勝率 |
| --- | --- | --- | --- |
| #28 詰み経済 | 動く | — | `Δpolicy` 最大 +0.0385（門 0.04） |
...
- **「4件とも反則が減った」ではない**。#34 は「分離しない」で、反則・勝率とも
  この形では測っていない。反則の削減を実測したのは #31（α）と #36（オラクル）の2件
