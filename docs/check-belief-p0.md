...
決定的なのは**これがオラクル = 上限**であること。P0-4 の門は
`R_learned / R_oracle ≥ 0.5` を要求し、そのうえで検証セットの継続で +0.04 を要求する。
**上限の時点で +0.034 しか出ない以上、学習可能な事前がその門を通る余地は無い**
（半分なら +0.017）。学習を作ってから落とすのではなく、ここで止めるのが正しい。
...
4件とも「機構は動く・反則は減る・勝率に変換されない」。#36 が違うのは、
**完全な信念（真の王手駒を知っている）というオラクル上限でもこの効果量**だったこと。
「王手中の反則を減らす」方向は、価格（#31 α）でも信念（#36）でも、
**そもそも勝率への変換率が低い**という読みが、これで2経路から支持された。
