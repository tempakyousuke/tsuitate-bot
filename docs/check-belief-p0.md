...
P1 へ進まずに閉じた。**閉じ方は3件で違う**ので一括りにしないこと:

- **#28 は受けの機構は動くが勝率の門に届かない**。**反則削減の施策ではなく**、
  むしろオラクルの継続は**反則負けが baseline 13% → 56% へ増える**
  （追加反則 平均 1.77回。`docs/mate-economy-p0.md` P0-6）
- **#31 は反則を減らした**（α が型1 を −53%）**が勝率の門に届かない**
- **#34 はそもそも分離しない**（反則・勝率とも未測定）

そのうえで3件の docs の「次にやるなら」が揃って**信念側**を指した。#31 P0-3 が出した破滅の署名は「知らない」ではなく**希釈**:
...
**コストは継続対局そのものが支配する**（当初「prewarm が支配する」と書いたのは
**誤りで、ジョブのログの実測で否定された**）。ログは `継続 N 局 / M 分` を出す:

| ジョブ | 決定点 | 継続 | 継続の時間 | 重い段 全体 | 継続の比率 |
| --- | ---: | ---: | ---: | ---: | ---: |
| v14 r0-s3 | 5 | 80本 | 70.6 分 | 75.7 分 | **93%** |
| v13 r0-s14 | 7 | 112本 | — | 159.0 分 | — |

**継続の本数 = 決定点 × seed 数 × arm 数**（5 × 4 × 4 = 80 / 7 × 4 × 4 = 112。
arm は `baseline` / `current` / `current@real` / `oracle@kinf@real` の4本）で、
1本あたり **53〜85 秒**（継続の平均思考 1363ms（v14）/ 1612ms（v13）、
反則負けで終わる長い継続が多いほど伸びる）。`entry_setup` の prewarm と
初回ランキングは残りの **4〜5 分**しかない。

したがって**シャードあたりの時間は「そのシャードの4局が持つ決定点の数」でほぼ決まる**
（局数が同じでも決定点が 5 と 7 なら 76 分と 159 分になる）。#31 P0-6（700ms・
40分/ジョブ）からの予算比スケールが外れたのは、予算のスケール自体ではなく
**決定点あたりの継続本数と1本の長さ**を見ていなかったため。

- **8 シャード（1シャード13局）は不足**（run 33333948875 の**最終**内訳。
  途中経過ではなく Actions の最終記録で数えたもの）:

  | 終了 | 本数 | 内訳 |
  | --- | ---: | --- |
  | 成功 | **8** | v14 6本 / **v13 2本**（r0-s5・r1-s5）。所要 **264〜313 分**（v14 だけなら 264〜310 分、最長は v13 r1-s5 の 312.8 分） |
  | 350分 timeout | **14** | 所要 350.2〜350.3 分・02:23 に一斉終了 |
  | 手動キャンセル | **10** | 06:44 に一斉終了（所要 261〜347 分でまだ終わっていない） |

  aggregate はシャード欠落で fail-closed なので、この run からは判定が出ない
...
**「完走したのは v14 ばかり」から v13 が桁違いに重いと読んではいけない**。
実際、**最終的には v13 も 2本完走している**（計測の途中で見ると v14 しか
終わっていない時間帯があり、**その途中経過を最終結果と取り違えたのが
このドキュメントの初版の誤り**）。v14 が 264〜310 分・v13 がその 1.1〜1.25 倍 =
290〜385 分なら、**350 分の境界をまたぐだけ**で完走の偏りは説明できる。
逆に**v14 だけでシャード数を決めてもいけない**（timeout した v13 は
...
## 実測: P0-2b（Check belief run 33351942982、commit 083d457）— **不通過。issue #36 は中止**

held-out 検証セット（Arena run 33331914813・`match_seed=20260830`・main vs v13 / v14 各104局・
commit 7bbb684）に対して、主 arm `oracle@kinf@real` を対照 `current@real` と比べた。
2000ms・4 seed・2 replicate・相手ごと24シャードで、**96シャードすべてと2つの
aggregate が success**、`continue-combined` だけが **exit 3（fail-closed）= 判定そのもの**。

```
== P0-2b 合算判定（主 arm oracle@kinf@real vs 対照 current@real）==
  estimand foul:   Δ差 +0.0343 [+0.0108, +0.0583] / 即時反則 -0.627 / 破滅 -0.0018 / 反則負け -0.0463 → 不通過
    estimator_v13: Δ差 +0.0373
    estimator_v14: Δ差 +0.0312
  estimand nofoul: Δ差 +0.0153 [-0.0102, +0.0430] / 即時反則 -0.054 / 破滅 +0.0000 / 反則負け -0.0150 → 通過
  **判定: 不通過** — estimand foul: 改善量か CI が門に届かない
```

- **門は「Δ ≥ +0.04 かつ CI 下限 > 0」の連言で、落ちたのは点推定のほう**。
  CI 下限 +0.0108 は正なので**効果は実在する**が、+0.0343 は門に 0.0057 足りない
- **相手別の符号 veto は通っている**（v13 +0.0373 / v14 +0.0312 で同符号）。
  安全性は margin の内側どころか**全部改善側**（即時反則 −0.627/手番・破滅 −0.18pt・
  反則負け −4.63pt）。`nofoul` も非劣性を満たす = **害は無い。効き目が足りないだけ**
- **CI は 0.04 を含むので「真の効果が +0.04 以上」は否定できない**。それでも
  事前登録の判定量は点推定であり、issue の「**門付近だけ増やす運用はしない**」が
  取り直しを禁じている（継続は常に2 replicate で、この値が既にその平均）。
  **不通過は不通過として確定させる**
- **相手別 report の「不合格」表示は informational**（PR #37 レビュー12巡目）。
  相手別は CI 下限まで正を要求する強い門になっていて、v14 の nofoul は
  CI 下限 −0.0252 で落ちるが、事前登録の判定は合算（−0.0102 > −0.02 で通過）。
  **合否を執行するのは `continue-combined` だけ**

### 中止の判断（issue #36 の事前登録どおり）

中止条件「P0-2b: オラクルの継続ペア差が +0.04 未満」に**該当する**ので、
**P0-3（学習可能性）へ進まない**。

**中止の根拠は事前登録した「P0-2b の継続ペア差が +0.04 未満」だけで足りる**。

補足として「これは上限だから学習版はさらに小さい」とも言えるが、**その議論に
`R_learned / R_oracle ≥ 0.5` を持ち出してはいけない**（PR #39 レビュー [P2]）:
`R` は**反則/手番の削減量**で、`R_oracle` は P0-2 の **+0.3340**。
P0-2b の **+0.0343 は継続スコア差（勝率単位）**で、**別の量**である。
「上限が +0.034 だから半分でも +0.017」は 2つの単位を混ぜた誤りで、
比率門からは「通る余地が無い」を導けない。

単位を揃えた正しい言い方はこうなる: P0-4 の最後の関門は**検証セットの継続で
+0.04**（P0-2b と同じ量）で、`oracle@kinf@real` は現行 support 内の局所的
reweighting の**上限**なので、学習事前の継続効果がそれを超えるとは考えにくい。
上限が +0.0343 である以上、学習版が +0.04 を通すには**上限を上回る必要がある**。
これは継続スコア差どうしの比較であって、比率門とは別の議論。

### 所見: #28 / #31 と同じく**門 0.04 に届かない**が、今度は**オラクル上限で**出た

| issue | 機構 | 反則 | 勝率 |
| --- | --- | --- | --- |
| #28 詰み経済（受け） | 動く | **増える**（オラクル継続の反則負け 13% → 56%） | `Δpolicy` 最大 +0.0385（門 0.04） |
| #31 王手中の反則経済 | 動く | α が −53% | `alpha@k2` +0.0060（門 0.04） |
| #34 被王手の前の準備 | 分離しない | — | — |
| **#36 王手駒仮説の希釈** | **動く（反則 1.39 → 0.36）** | **即時反則 −0.627/手番** | **+0.0343（門 0.04）** |

**表より強い主張をしないこと**（PR #39 レビュー [P2]）。正確には:

- **「4件とも反則が減った」ではない**。#34 は「分離しない」で反則・勝率とも
  この形では測っておらず、**#28 は反則削減の施策ですらない**（オラクルの継続は
  反則負けが 13% → 56% と**増える**）。反則の削減を実測したのは
  **#31（α）と #36（オラクル）の2件だけ**
- **#36 は「変換されない」ではない**。+0.0343・CI 下限 +0.0108 なので
  **正の変換は観測している**。正しくは「**正の変換は観測したが、事前登録した
  +0.04 の門に未達**」
- **#36 は「完全な信念」ではない**。`oracle@kinf@real` は
  **現行候補集合・現行探索内での局所的 reweighting のオラクル**であって、
  完全信念の上限ではない（issue #36 が呼称として明記している）。
  P0-1 の coverage failure 54.0% / 43.6% がその外側にある

そのうえで残る読みは、「王手中の反則を減らす」方向が価格（#31 α）でも
信念（#36）でも**勝率への変換率が低い**こと。これは2経路から示唆されるが、
**#34 は根拠に数えられない**。

**H1（事前の情報不足）は棄却も支持もされていない**: 仮説重みを直せば反則は確かに
減る（P0-2 の +0.334）。ここで測れたのは「**その削減は勝率へ正に変換される
（+0.0343・CI 下限 +0.0108）が、事前登録した +0.04 の門に届かない**」ことであって、
「勝率にならない」ではない。
