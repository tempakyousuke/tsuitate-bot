...
> 捕まえたい **−16.3pt の悪化を +8.0pt と逆向きに報告**した。
> 同じ構成の **A/A も +7.0pt [+0.2, +14.1]** で 0 を外している。
> 一方 **v14 vs v10 級の大差（+24.2pt）は正しく分離できる**。
> 詳細は「CI 規模の実測」と「撤退判断」。
...
**+7.0pt [+0.2, +14.1]** を出した。16局面のときも +18.8pt で、**2回とも候補側に振れている**。

`power simulation` に**偽陽性率（delta=0）の行**を足したので、以後は run ごとに
「判定規則が甘いのか、たまたま引いたのか」が名目 α との比較で読める
（delta=0 の経路は関数側に前からあったが、レポートから呼んでいなかった）。
...
| 撤退 | v12→v14 級の大差の符号すら安定しない、または通常 arena に明確なコスト優位が無い | **大差は出る**（v14 vs v10 +24.2pt [+15.2, +33.2]）。コスト優位は「数字上は 2〜3倍」だが**符号が合っていないので意味を持たない** |

### 現時点の結論（2026-08-24、700ms・較正1点）

1. **この構成（700ms・64局面 × 4 seed）は、issue が第一候補に置いた「破滅検出器」として
   機能しない**。捕まえたい −16pt 級を逆向きに +8pt と報告し、逃げ道に想定していた
   反則/局も逆を向いた
2. **A/A が有意に正へ振れる**（+7.0pt、16局面のときも +18.8pt）。バイアスが正側なら
   悪化を隠す向きなので、破滅検出器としては最悪。偶然か系統かは A/A をもう1本
   取れば決まる
3. **大差（v14 vs v10 級）は正しく分離できる**ので、「バグで壊れている」わけではない。
   壊れているのは −8〜−20pt 帯の分解能と符号の安定性。
   **判定規則（percentile cluster bootstrap CI）の偽陽性率も n=64 で 4.5〜5.8% と
   名目どおり**なので、統計の実装が甘いわけでもない
4. **通常 arena 側のブロッキングもほとんど効かない**（Var 0.97倍）。
   つまり「同じ条件を揃えれば分散が下がる」という前提自体が、この競技の
   勝敗指標には効きにくい。checkpoint の局面ブロッキングだけが効くという
   仮定にも慎重であるべき
5. **較正は1点しかない**。符号一致・順位相関は既知値2件以上でないと出せないので、
   「常に符号が逆」と一般化はできない。ただし**1点目が捕まえたい帯のど真ん中で
   逆を向いた**のは、続行の是非を問うに十分な材料
6. **通常 arena の運用と採否基準は変更していない**（CI は informational のまま）

**続行するなら A/A をもう1本が最優先**（約 8 CPU時間で「バイアスか偶然か」が決まる）。
ここで畳むなら、`arena-var`（通常 arena の局ごとのペア差・Var・安全性の共同指標）は
checkpoint arena と独立に有用なので残す。
