...
> **2026-08-24 の CI 規模実測の要点**: 配管と統計は動く（64局面 × 4 seed で
> SE ±3.4〜4.7pt、偽陽性率は n=64 で名目どおり、**v14 vs v10 級の大差 +24.2pt は
> 正しく分離できる**）。ただし **較正は成立していない**: 突き合わせた既知 arena 差は
> **2000ms**、checkpoint は **700ms** で、予算が違えば別の estimand なので
> 「符号が逆」とは言えない。**用途ごとの合否はまだ未判定**。
> A/A も 2本で +7.0pt / +0.0pt、持続的バイアスは確認できず不確定。
> 詳細は「CI 規模の実測」と「撤退判断」。
...
#### 較正は**成立していない**（思考予算が揃っていない）

**この突き合わせは無効**（PR #23 レビュー指摘1）。この文書自身が
「`--known-arena-delta` は**同じ candidate / control / opponent / 予算**で取り直した差
だけに付ける」と決めているのに、下の2つは予算が違う。

| | 思考予算 | 値 |
| --- | --- | ---: |
| 通常 arena のペア差（既知値のつもりだった） | **2000ms** | −16.3pt |
| checkpoint の delta | **700ms** | +8.0pt |

**予算が違えば別の estimand** なので、この2つを並べて「符号が逆」とは言えない。
`DROP_PROBE_REPEAT_GATE` の効果が 700ms と 2000ms で逆転している可能性が、
そのまま残っている（A/A は arm ラベルの対称性を見るだけで、
**ノブ × 思考予算の交互作用は検査しない**）。

参考として値だけ残すが、**P0 の完了判定にも撤退判断にも使わない**:

| 実験 | 予算 | delta | 95% CI |
| --- | --- | ---: | --- |
| 通常 arena（`cand_env=TSUITATE_DROP_PROBE_REPEAT_GATE=1`） | 2000ms | −16.3pt | [−28.8, −2.9] |
| checkpoint `drop_probe_gate` | 700ms | +8.0pt | [+0.2, +16.0] |
| 安全性: 反則/局（arena 2000ms） | 2000ms | +1.06 | [+0.30, +1.82] |
| 安全性: 反則/局（checkpoint 700ms） | 700ms | −0.28 | – |

**成立する較正を取るには、どちらかへ予算を揃える**（issue #19 の
「未較正の短い予算を 2000ms の強さと見なさない」「700/2000 で符号を較正する」）:

- checkpoint を **2000ms** で取り直す（約 18 CPU時間。既存の −16.3pt がそのまま使える）
- 通常 arena を **700ms 相当**で取り直す（約 5 CPU時間。`-f env=TSUITATE_THINK_BUDGET_MS=700`
  で両側に効かせると、checkpoint の `--budget-ms 700` と同じ条件になる）

**この誤りを機械的に止められなかったのは `arena-var` の検査不足**でもある。
game row に相手の実効挙動の指紋も両側の実効予算も残しておらず、cross-arm 検査は
相手の**名前**と時計しか見ていなかった。schema 2 で必須にして、予算不一致は
`--allow-budget-diff` を明示しない限り止めるようにした（同レビュー指摘2）。

なお **大差は正しく出る**（予算の揃った同一 run 内の比較なので、これは有効）:
v14 vs v10 は +24.2pt [+15.2, +33.2]、初手不一致率も 52.3% と他の実験（17〜28%）より
明らかに高い。issue の撤退条件「v12→v14 級の大差の符号すら安定しない」には**当たらない**。

#### A/A: 1本目は偽陽性、2本目は 0 中心
...
**判定規則そのものは正しく較正されていた**。偽陽性率の実測（各実験の経験分布から
再標本化、600回 × bootstrap 400回、名目 5%）:
...
**n=64 では 4.5〜5.8% で名目どおり**（n=16 では 7.5〜11.8% と甘く、
16局面の構成を使ってはいけないことの裏づけにもなる）。

つまり A/A の +7.0pt は **bootstrap の甘さでは説明できない**。残る説明は
「名目 5% の偽陽性をたまたま1回目で引いた」か「候補 arm への系統バイアスが実在する」
のどちらか。**A/A をもう1本取れば決まる**（約 8 CPU時間）。

**バイアスが正側だとしたら、破滅検出器としては最悪の向き**である
（真の −7pt の悪化が 0 に見えて素通りする）。実際 drop_probe_gate の
−16.3pt → +8.0pt はその形になっている。

### P0 の残り

**CI 規模の計測は 2026-08-24 に回した**（上記）。ただし較正は
**予算が揃っていないため成立していない**ので、P0 の完了条件はまだ埋まっていない。

- [x] **通常 arena のペア差の実測 Var** → `Var(ペア差) 0.4682`（独立仮定の 0.97 倍 =
      ブロッキングはほぼ効かない）・1ペア 254 CPU秒・`var·CPU秒 119`（2000ms）。
...
- [ ] **既知差の較正** → **未成立**。突き合わせた2つの予算が違う
      （arena 2000ms vs checkpoint 700ms）。どちらかへ揃えて取り直す:
      **arena を 700ms 相当で取り直す（約 5 CPU時間）**か、
      **checkpoint を 2000ms で取り直す（約 18 CPU時間）**
- [ ] 700ms と 2000ms の符号・順位・副指標の方向の一致 → **未着手**
      （較正が成立していないので、そもそも比べる相手が無い）
- [x] **arena 記録から作ったデッキ**での測定（run 32697854659 の記録から 64件）
- [x] A/A の再現性 → 2本取った（+7.0pt / +0.0pt）。**持続的バイアスは確認できず、
      現状は不確定**

## 撤退判断

issue が置いた基準と、**CI 規模の実測（2026-08-24、700ms）を当てた結果**。
**較正が成立していないので、用途ごとの合否はまだ確定していない**
（突き合わせた既知値が 2000ms、checkpoint が 700ms。PR #23 レビュー指摘1）。

| 用途 | 基準 | 現時点 |
| --- | --- | --- |
| quick 破滅検出器 | −15〜−20pt 級を通常 arena の概ね 1/3 以下の CPU で検出 | **未判定**。700ms の checkpoint は +8.0pt を出したが、比較相手の −16.3pt は 2000ms なので合否を決められない |
| validation ゲート | −8〜−13pt 級を概ね 1/2 以下の CPU で検出 | **未判定**（同上） |
| 安全性ゲートへの縮小 | 既知の重大悪化を反則/局等が安定して検出でき、費用が小さい | **未判定**。反則/局も arena +1.06（2000ms） vs checkpoint −0.28（700ms）で予算が揃っていない |
| 撤退 | v12→v14 級の大差の符号すら安定しない、または通常 arena に明確なコスト優位が無い | **大差は出る**（v14 vs v10 +24.2pt [+15.2, +33.2]。予算の揃った同一 run 内の比較なので有効）。コスト優位は数字上 var·CPU秒 31〜57（700ms）vs arena 119（2000ms）だが、**予算も尺度（壁時計 vs 思考時間）も違うので比較になっていない** |

### 現時点で言えること（2026-08-24、合計 38 CPU時間）

1. **配管と統計は動く**。64局面 × 4 seed で SE ±3.4〜4.7pt / ICC 0.00〜0.32、
   偽陽性率は n=64 で 4.5〜6.7%（名目 5%）。32ジョブ全成功・片割れ欠損 0
2. **大差（v14 vs v10 +24.2pt）は正しく分離できる**。issue の撤退条件
   「大差の符号すら安定しない」には当たらない
3. **A/A は 2本で +7.0pt / +0.0pt**。点推定 +3.5pt・run 間の開き 7.0pt で、
   **持続的なバイアスは確認できず、現状は不確定**。1本目の原因は確定していない
4. **較正はまだ1件も成立していない**。予算を揃えて取り直すまで、
   「checkpoint の delta が通常 arena と対応するか」は**分かっていない**。
   700ms でノブの効果が逆転している可能性も、対応しない可能性も、どちらも残っている
5. **通常 arena 側のブロッキングはほとんど効かない**（Var 0.4682 = 独立仮定の 0.97倍）。
   これは予算の揃った 2000ms のペア内での実測なので有効。「同じ条件を揃えれば
   分散が下がる」という前提自体が、この競技の勝敗指標には効きにくい
6. **通常 arena の運用と採否基準は変更していない**（CI は informational のまま）

### 次にやること

**較正を成立させるのが先**。それまで用途ごとの合否は判定できない。

| 案 | コスト | 何が分かるか |
| --- | ---: | --- |
| **通常 arena を 700ms 相当で取り直す**（`-f env=TSUITATE_THINK_BUDGET_MS=700` の対照＋候補ペア） | 約 5 CPU時間 | checkpoint 700ms と**同じ estimand** の既知値。最も安く較正が成立する |
| checkpoint を 2000ms で取り直す | 約 18 CPU時間 | 既存の −16.3pt をそのまま使える。本番の予算での較正になる |
| 較正点を増やす（`ANCHOR_MOVE_W=0.6` 等） | 1点あたり arena ペア + checkpoint | **符号一致・順位相関は既知値2件以上でないと出ない** |
| `--min-remaining` を伸ばす（例 60手） | デッキ作り直し + 8 CPU時間 | 「継続が短すぎて累積の害が出ない」仮説の検証 |
