...
> **2026-08-24 の CI 規模実測の要点**: 700ms・64局面 × 4 seed で、
> 捕まえたい **−16.3pt の悪化を +8.0pt [+0.2, +16.0] と逆向きに報告**した。
> 統計は健全である（A/A 2本目は **+0.0pt [−6.6, +6.6]**、偽陽性率も n=64 で名目どおり）。
> **v14 vs v10 級の大差（+24.2pt）は正しく分離できる**。
> つまり問題はノイズでも判定の甘さでもなく、**測っている量が通常 arena の勝敗差と
> 対応していない**こと。詳細は「CI 規模の実測」と「撤退判断」。
...
**同じ candidate / control / opponent で測り直した既知値と突き合わせたのは 1 件**。

| 実験 | 既知 arena 差（実測） | checkpoint（700ms） | 判定 |
| --- | ---: | ---: | --- |
| drop_probe_gate | **−16.3pt** | **+8.0pt** | **符号が逆** |
| aa（真値 0） | 0pt | +7.0pt（CI が 0 を外す） | **偽陽性** |

**捕まえたい −16pt 級の悪化を、逆向きに +8.0pt と報告した。** A/A のオフセット +7.0pt を
引いても +1.0pt で、負にすらならない。

**安全性の共同指標も逆を向いた**: arena は反則/局 **+1.06**（悪化）なのに、
checkpoint は **−0.28**（改善に見える）。「勝敗が鈍くても反則/局なら鋭い」という
逃げ道（安全性ゲートへの縮小）もこの構成では成立しない。

一方、**大差は正しく出る**: v14 vs v10 は +24.2pt [+15.2, +33.2] で、初手不一致率も
...
**2本目はぴたりと 0 中心**（改善と悪化がちょうど 55 対 55）。つまり
**候補 arm への系統バイアスは無く、1本目は名目 5% の偽陽性を引いたということ**。
...
**これは checkpoint arena にとって良い知らせではない**。統計も実装も健全だと確かめられた
うえで、なお **既知 −16.3pt の悪化を +8.0pt [+0.2, +16.0] と報告した**からである。
つまり問題は「ノイズが大きい」でも「判定が甘い」でもなく、
**測っている量そのものが通常 arena の勝敗差と対応していない**。
...
**CI 規模の計測は 2026-08-24 に回した**（上記）。残っているのはここから先の判断と、
必要なら追加で取る計測。

- [x] **通常 arena のペア差の実測 Var** → `Var(ペア差) 0.4682`（独立仮定の 0.97 倍 =
      ブロッキングはほぼ効かない）・1ペア 254 CPU秒・`var·CPU秒 119`。
      計測経路は `ARENA_GAMES_JSON` / `arena-var` / `pair_with`
- [x] 64 checkpoint・**seed 4**（= 2 replicate）での empirical SE / ICC / MDE →
      SE ±3.5〜4.7pt / ICC 0.00〜0.32 / MDE ±9.9〜13.1pt
- [x] 既知差の較正 → **1点だけ実施し、符号が逆だった**（drop_probe_gate:
      arena −16.3pt vs checkpoint +8.0pt）。`report` の符号一致・順位相関は
      既知値2件以上でないと出ないので、まだ「未較正」表示のまま
- [ ] 700ms と 2000ms の符号・順位・副指標の方向の一致 →
      **700ms で符号が合わなかったので、2000ms を回す価値があるかは判断待ち**
- [x] **arena 記録から作ったデッキ**での測定（run 32697854659 の記録から 64件）

追加で取る価値があるとしたら次の3つ。どれも**やる前にコストと得られる答えを見比べること**。

1. **A/A をもう1本**（別 `--seed-base`。約 8 CPU時間）: +7.0pt が再現するなら系統バイアス、
   符号が反転するなら偶然。**この1本で「バイアスか偶然か」が決まる**ので、
   続行するなら最優先
2. **2000ms での較正**（約 25 CPU時間 + arena ペアは取得済み）: 700ms 固有の問題である
   可能性を潰す。ただし A/A のバイアスは予算と独立に出ている可能性が高い
3. **較正点を増やす**（`ANCHOR_MOVE_W=0.6` 等。1点あたり arena ペア 8 CPU時間 +
   checkpoint 8 CPU時間）: 符号一致・順位相関を出すには既知値が2件以上要る
...
| 用途 | 基準 | 実測での判定 |
| --- | --- | --- |
| quick 破滅検出器 | −15〜−20pt 級を通常 arena の概ね 1/3 以下の CPU で検出 | **不合格**。−16.3pt の悪化を **+8.0pt** と逆向きに報告した |
| validation ゲート | −8〜−13pt 級を概ね 1/2 以下の CPU で検出 | **不合格**（quick より厳しい条件なので自動的に） |
| 安全性ゲートへの縮小 | 既知の重大悪化を反則/局等が安定して検出でき、費用が小さい | **不合格**。arena は反則/局 +1.06（悪化）、checkpoint は −0.28（改善に見える）で符号が逆 |
| 撤退 | v12→v14 級の大差の符号すら安定しない、または通常 arena に明確なコスト優位が無い | **大差は出る**（v14 vs v10 +24.2pt [+15.2, +33.2]）。コスト優位は「数字上は 2〜3倍」だが**符号が合っていないので意味を持たない**（var·CPU秒 31〜57 vs arena 119。ただし壁時計 vs 思考時間・700ms vs 2000ms で尺度が違う） |

### 現時点の結論（2026-08-24、700ms・較正1点・合計 38 CPU時間）

1. **この構成（700ms・64局面 × 4 seed）は、issue が第一候補に置いた「破滅検出器」として
   機能しない**。捕まえたい −16.3pt を逆向きに +8.0pt と報告し、逃げ道に想定していた
   反則/局も逆を向いた
2. **統計は健全**。A/A 2本目は **+0.0pt [−6.6, +6.6]**（改善55 / 悪化55）で
   系統バイアスは無く、1本目の +7.0pt は名目 5% の偽陽性を引いただけだった。
   偽陽性率の実測も n=64 で 4.5〜6.7% と名目どおり
3. **大差（v14 vs v10 級 +24.2pt）は正しく分離できる**。issue の撤退条件
   「大差の符号すら安定しない」には当たらない
4. **2 と 3 が成り立つのに 1 が起きる**、というのがこの P0 の答えである。
   問題は「ノイズが大きい」でも「判定が甘い」でもなく、
   **checkpoint の delta が通常 arena の勝敗差と対応していない**。考えられる筋は3つ:
   ...
   - checkpoint の局面は**対照の方策が作った局面**なので、候補にとっての有利／不利が
     偏りうる（選択バイアス。同じデッキを両 arm に使っても消えない）
   - 700ms 固有（2000ms なら違う）。ただし A/A が健全である以上、これだけで
     符号が逆になる説明にはなりにくい
5. **通常 arena 側のブロッキングもほとんど効かない**（Var 0.4682 = 独立仮定の 0.97倍）。
   「同じ条件を揃えれば分散が下がる」という前提自体が、この競技の勝敗指標には
   効きにくい。checkpoint の局面ブロッキングだけが効くという仮定にも慎重であるべき
6. **較正は1点しかない**ので「常に符号が逆」と一般化はできない。ただし
   **1点目が捕まえたい帯のど真ん中で逆を向いた**以上、この構成のまま
   ゲートに使うことはできない
7. **通常 arena の運用と採否基準は変更していない**（CI は informational のまま）

### 続けるとしたら

**A/A の再現（最優先だった項目）は済んだ**ので、残るのは「なぜ対応しないのか」の切り分け。
どれもやる前にコストと得られる答えを見比べること。

| 案 | コスト | 何が分かるか |
| --- | ---: | --- |
| 2000ms で同じ較正を回す | 約 25 CPU時間（arena ペアは取得済み） | 700ms 固有かどうか。A/A が健全なので優先度は下がった |
| 較正点を増やす（`ANCHOR_MOVE_W=0.6` 等） | 1点あたり arena ペア 8 + checkpoint 8 CPU時間 | 「1点目がたまたま逆だった」のか「系統的に対応しない」のか。**符号一致・順位相関を出すには2件以上必要** |
| `--min-remaining` を大きく取る（例 60手） | 既存デッキの作り直し + 8 CPU時間 | 「継続が短すぎて累積の害が出ない」仮説の検証 |
