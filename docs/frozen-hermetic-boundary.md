...
なお、この判定を検査するテストは**プロセス env を注入で受ける純粋版**
（`*_with_env` / `EnvSource::empty()` を base にする）を呼ぶ。プロセス env を
テスト内で set/remove すると並列テストと競合し、そのまま読むと呼び出し元の
shell に残った `TSUITATE_*` でテストが落ちる。
