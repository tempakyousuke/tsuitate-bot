...
**フェーズ2（2026-07-22〜23）で bot 本体への推論統合が完了し、estimator_v10
として凍結した**（本ドキュメント末尾の「フェーズ2: 本番統合」参照）。
...
## フェーズ2: 本番統合（2026-07-22〜23、estimator_v10 凍結）

発端はユーザーの実戦対局レビュー（54手目9二香: 意味を問わない advance_bias
だけで手が決まる「浅い1手読み評価の横並び」問題）。フェーズ1の区切りから
統合へ直行する路線をユーザーが選択した。

**実装**（ブランチ `nn-value-integration`）:
- `tsuitate-nn/export_value_weights.py`: train.py の out/（model.pt +
  norm_stats.json）を Rust 定数へ書き出し（opp_move と同じ AUTO-GENERATED
  区間方式。学習は train.py に一本化したまま）
- `src/value_nn.rs`: 手書き forward pass（22→64→32→1、release 実測 0.6µs/回）
  ＋ゼロ重みガード・numpy クロスチェック・有界性・速度のテスト4件
- `strategy.rs::evaluate()`: 粒子ループで NN を推論し重み付き平均。
  state 特徴量は粒子単位に遅延キャッシュ（候補間で共通）、`nn_samples`
  （予算比例、基準48）で粒子数制限、NN項は gain 内（p_legal 割引の内側）

**学習データの再生成**: フェーズ1の3000局データは定石更新（2026-07-22）前の
ものなので、新定石＋estimator/v8/v9 教師の 512局×3（run 29918060369）から
再生成（value 114k行 + pairwise 106kペア）。4シード学習で gold-check は
4/4 正解（マージン+0.12〜0.17）、kakudo は seed1 のみ正解 → seed1 採用。

**w 選定スイープ**（w=3/6/10 × 5シナリオ、各20試行）の発見:
- w=3（オフラインのスコア差×3 ≒ 0.5歩）では gold-check の悪手 17/20 が
  変わらない。w=6 で 2/20、w=10 で 0/20 に反転 → **w=6 採用**
- w≥6 は王手中の反則試行を増やし dragon-check-drop で反則負け 2/20 が発生
  → **you_in_check 中は NN 項を無効化**（王手回避は CheckSolver の領分）。
  ゲート後: 正解 20/20・追加反則 16件（NNオフの19件より少ない）

**凍結時の成績**: vs v6 77.0%±5.8 / vs v7 69.8%±6.4 / vs v8 56.9%±3.4
（800局合算 454-344-2。初回200局が 55.3%±6.9 で未確定 → +200局 → +400局と
拡張して確定）/ vs v9 57.3%±6.9 — 全凍結版に有意勝ち越し。
scenario suite 回帰なし（gold-check 悪手 5/20、他は維持）。

**残る限界**: 「無目的な安全手同士の序列」（9二香のような横並び）は新モデル
でも判別しない（9a9b 選択 6〜9/20 のまま）。value ネットが効くのは
「明確な悪手の回避」であり、方針・計画性の欠如そのものはまだ未解決。
次の候補: hard pairwise mining・多手先ラベル（TD）・方策事前分布の学習など。
