...
水準は比べられない。読むのは**残数に対する曲線の形**だけ"
    ...
    // 実行順の均衡と、半分ごとの影の価格。
    // **加法的な実行順の主効果 ω なら**、AB/BA が釣り合っていればペア差の平均から
    // 厳密に落ちる（treatment 先は δ+ω、control 先は δ−ω を観測するため）。
    // したがって ω̂ ≠ 0 それ自体は推定の歪みを意味しない。
    // **ただし半分ごとの価格差は関門ではない**: 加法的主効果があれば 2ω 食い違うのが
    // 正常で、しかも実行順 × treatment の交互作用も同じ形で入るため両者を識別できない。
    // 加法性はこの設計では検証できない仮定である（PR #33 レビュー2巡目 [P2]）。
    order_balance(&all);
    ...
    // **実行順効果**（先に走った arm − 後に走った arm）= 環境に順番の効果が有るかの量。
    // 加法的な主効果なら均衡で相殺されるので、0 から離れていること自体は
    // 「均衡が効いていない」を意味しない（交互作用とは識別できないのが本当の限界）
    let order_effect = {
