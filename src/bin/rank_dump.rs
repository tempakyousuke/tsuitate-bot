...
//!
//! `evals/*.eval.md` 向けの「終局まで bot の候補手を評価していく」レビューの
//! スケルトン生成用（scenario-gui の「ランキング」の一括版。現行の一次資料は
//! eval。`docs/quest_20260731.md` は移行前の経緯記録）。同一手番側は
                ...
                match snap.last_ranking() {
                    Some(ranking) => {
                        for c in ranking.iter().take(top_n) {
                            if show_scores {
                                println!(
                                    "{}({}) score={:.3} gain={:.3} static_gain={:.3} capture={:.3} promo={:+.3} pbias={:+.3} dbias={:+.3} link={:+.3} p_legal={:.2} adjust={:+.3} depth2={}",
                                    jp_move(pos, &c.usi),
                                    c.usi,
                                    c.score,
                                    c.gain,
