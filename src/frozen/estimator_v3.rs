//! estimator の凍結版 v3（2026-07-06 凍結）。
//!
//! **このファイルは編集しない**（frozen/mod.rs の運用ルール参照）。
//! v2 との差分: リプレイ再生成の限定バックトラック — 相手手のサンプルが後続の
//! 観測と矛盾したとき、初手からやり直すのではなく直近の決定点（相手手）へ戻って
//! 引き直す。手数が伸びたときのリプレイ成功率を上げ、粒子枯渇を減らす。
//!
//! 参考強度: vs estimator_v2 62.9%±8.2%（200局、平均反則 5.10 vs 6.51、2026-07-06）
...
/// estimator v3（凍結）。観測履歴から相手局面を推定し、候補手を粒子平均で評価する
pub struct EstimatorV3 {
    est: Option<Estimator>,
}

impl EstimatorV3 {
    pub fn new() -> Self {
        EstimatorV3 { est: None }
    }
}

impl Default for EstimatorV3 {
    fn default() -> Self {
        Self::new()
    }
}

impl Strategy for EstimatorV3 {
    fn choose(
        &mut self,
        view: &PlayerView,
        log: &ObservationLog,
