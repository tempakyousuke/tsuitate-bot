//! estimator の凍結版 v7（2026-07-18 凍結）。
//!
//! v6 からの差分 = C-7「連続重み粒子フィルタ」P1+P2（設計と経緯は
//! docs/c7-continuous-filter.md。codex+cursor の設計レビュー、codex の
//! 実装レビューを経て確定）:
//! - P1: penalty→info_miss（別勘定カウンタ化）、ソフト救済の減衰を評価側
//!   soft_decay^penalty から logw への ln(EPS_INFO) 課金へ統合、ESS 監視つき
//!   systematic resampling と質量保存の分割複製、評価側 stratified_sample の
//!   multiplicity 畳み込み（リサンプリング後は複製数が事後質量）と
//!   較正アンカー再定義（EPS_INFO^min_miss、複製は独立証拠に数えない）
//! - P2: 窓付き若返り（直近8相手決定点のスナップショットへ巻き戻して
//!   制約後読みガイド付きで引き直す。depth-major バッチ）、Guide{lands,attacks}、
//!   補正 Δlogw = ln(r)+ln(p_class/g_class)、墓場復活（厳密全滅時の保管と
//!   以後ターンでの復活）、logw の rebase_cidx（リサンプリング後のスケール整合）
//! - 粒子尤度 exp(θ·φ)（likelihood.rs 2026-07-16 フィット）と SIR 観測尤度
//!   （keima 対応）は v6 凍結後に main へ入った分を含む
//!
//! 凍結時の成績（200局 match_seed=20260718、v6のみ 別run 20260718）:
//! vs v6 64.2%±6.8 / vs v5 83.8%±5.5 / vs v4 82.7%±5.5 /
//! vs v3 88.0%±4.7 / vs v2 89.3%±4.5。反則 3.3〜5.9/局（基準側 6.6〜7.7）
//!
...
use crate::board::{
    Coord, Promotion, drop_targets, make_usi_drop, make_usi_move, make_usi_square,
...
impl Default for EstimatorV7 {
    fn default() -> Self {
        Self::new()
    }
}

impl Strategy for EstimatorV7 {
...
impl EstimatorV7 {
    ...
    pub fn with_seed(seed: u64) -> Self {
        EstimatorV7::with_params_line_seed(EvalParams::default(), None, Some(seed))
    }
}
