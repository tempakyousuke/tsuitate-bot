//! estimator の凍結版 v9（2026-07-22 凍結）。
//!
//! v8 からの主な差分（NN段階①-a/①-b、ブランチ nn-opp-move-weight-selfplay）:
//! - **opp_move_weight のNN化（①-a）**: bin/fit_opp の手作り線形フィットを
//!   1隠れ層MLP（手書きforward pass、外部クレート依存ゼロ）へ置き換え。
//!   学習データはbot自己対局のアリーナ記録（教師は凍結版側の実手）。
//!   反則回数特徴量 opp_foul_count_this_turn（相手手番内の反則試行回数、
//!   実戦でも観測可能）をライブ配線（Constraint::OppMove に foul_count、
//!   Observation::OpponentFoul を累積して次の OppMove 確定時に焼き込み）
//! - **駒種特化特徴量（①-b、v9の主変更）**: 駒種one-hot7種・成駒フラグ・
//!   移動距離・from_home（初期配置マスからの移動 = home_lance_move の
//!   全駒種一般化）を追加し13→23次元。①-a撤退の決定打だった「NNが駒種を
//!   区別できず kakutori の角長距離王手に信念が立たない」欠陥への直接対応。
//!   kakutori 注目手 2/10→10/10、真の王手駒 Bishop@1e 信念 1.0%→2.7%
//! - 旧 home_lance_move の別立て -1.3 加点は from_home×lance で表現可能に
//!   なったため廃止（自己対局データでは凍結版の from_home 香車選択率は
//!   平均並みで、割り引き信号自体が存在しない）
//! - NN出力は .clamp(-15,15)（リプレイ仮説探索中の分布外入力での暴走防止）
//! - 学習データは定石更新（joseki.json 拡充、2026-07-22）後の新定石で
//!   再生成した1536局（run 29853957855）。定石は凍結版側の序盤分布も
//!   変えるため、定石更新時はデータ再生成が必要（教訓）
//!
//! 凍結時の成績（200局×3基準・GitHub Actions run 29887306320、2026-07-22）:
//! vs v6 78.0%±5.7（156-44-0）/ vs v7 62.3%±6.7（124-75-1）/
//! vs v8 57.0%±6.9（114-86-0）— 全凍結版に有意勝ち越し
//!
...
// ---------------------------------------------------------------------------
// opp_move NN（opp_move_nn.rs のコピー、①-b 23次元・新定石1536局で学習）
// ---------------------------------------------------------------------------
...
const OPP_MOVE_NN_W1: [[f64; 23]; 16] = [
...
const OPP_MOVE_NN_B1: [f64; 16] = [-2.03199649e+00, -4.74606931e-01, -7.00383663e-01, -6.86869383e-01, -1.40031195e+00, -7.84394324e-01, -3.84654373e-01, -2.05938935e+00, -4.81621295e-01, -3.64736557e-01, -7.69063309e-02, -1.73976672e+00, -5.19845307e-01, -2.56025016e-01, 1.40186083e+00, -7.20858634e-01];
const OPP_MOVE_NN_W2: [f64; 16] = [2.54354656e-01, 1.38985917e-01, 2.43878514e-01, 2.49244422e-01, -3.27984959e-01, 2.60426015e-01, 2.17521861e-01, -3.41697901e-01, -3.73856157e-01, -2.88700551e-01, -4.90161419e-01, 3.54575515e-01, 2.85066701e-02, -3.09361517e-01, -2.48380899e-01, 2.32507974e-01];
const OPP_MOVE_NN_B2: f64 = -1.19723612e-03;
// AUTO-GENERATED END

/// 学習時と同じ正規化 + Linear(23→16) → ReLU → Linear(16→1) のforward pass。
/// 出力はlogit（Sigmoidではない）。呼び出し側は `exp(logit)` として使う
fn opp_move_nn_forward(features: &[f64; 23]) -> f64 {
    let mut x = [0.0f64; 23];
    for i in 0..23 {
...
impl Default for EstimatorV9 {
    fn default() -> Self {
        Self::new()
    }
}

impl Strategy for EstimatorV9 {
...
impl EstimatorV9 {
    ...
    pub fn with_seed(seed: u64) -> Self {
        EstimatorV9::with_params_line_seed(EvalParams::default(), None, Some(seed))
    }
}
