...
fn evaluate(
    ...
    //
    // **王手中は無効**（value_nn / 詰めろ2項 / capture_bet_var と同じゲート）。
    // 王手中の序列は解消確率（CheckSolver）が支配すべきで、盤面全体の紐という
    // 局面全体の量を足すと解消手の相対差が埋もれる。実測（kakutori.kif、
    // 角の王手を無条件に取れる局面）で捕獲 17/20 → **3/20** の回帰が出た:
    // 取る手（1d1e）は駒が守りの網から離れるぶん紐を失い、王手を放置して
    // 紐を保つ手（3g2g）に負けていた
    let link = if view.you_in_check {
        0.0
    } else {
        params.link_w * own_effects.linked_value
    };
